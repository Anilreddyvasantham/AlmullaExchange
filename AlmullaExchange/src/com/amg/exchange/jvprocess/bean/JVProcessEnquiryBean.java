...
@Component("jvProcessEnquiryBean")
@Scope("session")
public class JVProcessEnquiryBean<T> implements Serializable{

	private static final long serialVersionUID = 1L;

	public static final Logger log = Logger.getLogger(JVProcessEnquiryBean.class);
	 ...
	 public void  populateAccountNumber(BigDecimal bankId,BigDecimal currencyId){
		...
		public void approval(){
		  ...
		  public void clickOk(){
			...
			public void navigateToJvProcessEnquiryPage()
			{
				clear();
				setDocumentId(null);
				try {
					loginLogoutHistoryUtil.saveLoginLogoutDetails(sessionStateManage.getCountryId(), sessionStateManage.getUserType(), sessionStateManage.getUserName(), "jvprocessenquiry.xhtml");
					FacesContext.getCurrentInstance().getExternalContext().redirect("../jvprocess/jvprocessenquiry.xhtml");
				} catch (IOException e) {
					e.printStackTrace();
				}
			...
			public void populateUnapprovalList(){
				List<DayBookHeader> listDayBook = iJVProcessService.getDocumentNoForEnquiry(getDocumentId());
