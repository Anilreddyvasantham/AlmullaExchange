...
@Component("promotionDeclarationMBean")
@Scope("session")
public class PromotionDeclarationMBean<T> implements Serializable  {
	...
	public void pageNavigation() {
		try {
			...
			FacesContext.getCurrentInstance().getExternalContext().redirect("../promotion/promotionDeclaration.xhtml");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	{
		PromotionWinnerDataTable promotionWinnerDataTable=(PromotionWinnerDataTable) event.getObject();
		setSelectedPromotionWinnerDataTable(promotionWinnerDataTable);
	...
	public void savePromotionalDetails()
	{
		try{
			...
			{
				RequestContext.getCurrentInstance().execute("notSelected.show();");
				return;
			}
			if(getDigitalSignature()!=null && !getDigitalSignature().equalsIgnoreCase(""))
			{
				...
				if(promotionWinnerDataTable!=null)
				{
					java.sql.Clob custSignature =stringToClob(getDigitalSignature());
					iPromotionService.savePromotionDeclaration(promotionWinnerDataTable.getPromotionDetailsId(), custSignature, sessionStateManage.getUserName());
					FacesContext.getCurrentInstance().getExternalContext().redirect("../promotion/promotionDeclarationReport.xhtml");					
				}
			}else
			{
				RequestContext.getCurrentInstance().execute("noSingnature.show();");
			}
	...
	public void genaratePromoDeclarationReport()
	{
		ServletOutputStream servletOutputStream =null;
		try {
			fetchPromotionDeclarationReport();
	...
	private void fetchPromotionDeclarationReport() {
